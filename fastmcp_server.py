...
@mcp.tool()
def create_vm_complete(template_name: str, new_vm_name: str, datastore_name: str = None, memory_gb: int = None, cpu_count: int = None, disk_gb: int = None, network_name: str = None) -> str:
    """Create a new VM from template with complete customization using the working approach."""
    if not connect_to_vcenter():
        return "Error: Could not connect to vCenter"
...
@mcp.tool()
def create_vm_robust(template_name: str, new_vm_name: str, datastore_name: str = None, memory_gb: int = None, cpu_count: int = None, disk_gb: int = None, network_name: str = None) -> str:
    """Create a new VM from template with robust error handling and customization."""
    if not connect_to_vcenter():
        return "Error: Could not connect to vCenter"
    ...
    try:
        ...
        # Create relocation spec (exactly like create_vm_from_template)
        relospec = vim.vm.RelocateSpec()
        if datastore:
            relospec.datastore = datastore
        
        # Create config spec (exactly like create_vm_from_template)
        configspec = vim.vm.ConfigSpec()
        configspec.location = relospec
...
@mcp.tool()
def create_vm_safe(template_name: str, new_vm_name: str, datastore_name: str = None, memory_gb: int = None, cpu_count: int = None, disk_gb: int = None, network_name: str = None) -> str:
    """Create a new VM from template with safe approach - no location spec to avoid resource pool issues."""
    if not connect_to_vcenter():
        return "Error: Could not connect to vCenter"
    ...
    try:
        ...
        # Create config spec WITHOUT location (this is the key difference)
        configspec = vim.vm.ConfigSpec()
        ...
        # Clone the VM WITHOUT location spec (uses template's default location)
        task = template.Clone(folder=template.parent, name=new_vm_name, spec=configspec)
        ...
        if task.info.state == vim.TaskInfo.State.success:
            result = f"✅ Successfully created VM '{new_vm_name}' from template '{template_name}'"
            if datastore_name:
                result += f"\n- Datastore: {datastore_name} (inherited from template)"
            if memory_gb:
                result += f"\n- Memory: {memory_gb} GB"
            if cpu_count:
                result += f"\n- CPU: {cpu_count} cores"
            ...
            if network_name:
                result += f"\n- Network: {network_name}"
            result += f"\n\n💡 Note: VM uses template's default datastore and resource pool. IP address will be assigned via DHCP."
            return result
        else:
            return f"❌ Failed to create VM: {task.info.error.msg}"
...
@mcp.tool()
def create_vm_official(template_name: str, new_vm_name: str, datastore_name: str = None, memory_gb: int = None, cpu_count: int = None, disk_gb: int = None, network_name: str = None) -> str:
    """Create a new VM from template using official VMware pyVmomi pattern."""
    if not connect_to_vcenter():
        return "Error: Could not connect to vCenter"
    ...
    try:
        ...
        # Create the relocation spec (official VMware pattern)
        relospec = vim.vm.RelocateSpec()
        if datastore:
            relospec.datastore = datastore
        
        # Create the clone spec (official VMware pattern)
        ...
        # Add memory and CPU customizations
        if memory_gb:
            config_spec.memoryMB = memory_gb * 1024
        if cpu_count:
            config_spec.numCPUs = cpu_count
        ...
        # Attach config spec to clone spec (official VMware pattern)
        clonespec.config = config_spec
        
        # Clone the VM (official VMware pattern)
